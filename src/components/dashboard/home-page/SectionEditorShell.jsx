...
/**
 * Shell that handles fetching, saving, and layout for a single home section editor.
 *
 * Props:
 *  - sectionKey: string  e.g. 'banner'
 *  - title: string
 *  - description: string
 *  - children: (data, setData, saving) => ReactNode
 *    — render prop receiving current section data, setter, and saving state
 */
export default function SectionEditorShell({ sectionKey, title, description, children }) {
  ...
  const load = useCallback(async () => {
    ...
    setError(null);
    try {
      const res = await fetch(`/api/home-content?section=${sectionKey}`, { credentials: 'include' });
  ...
  return (
    <div className="max-w-[860px] mx-auto">
      ...
      <div className="flex items-center gap-2 text-[12px] text-[#77838f] mb-4 flex-wrap">
        ...
        <Link href="/dashboard/page-content/home" className="hover:text-[#eb5905] transition">Home Page</Link>
        <ChevronRight className="w-3.5 h-3.5 shrink-0" />
