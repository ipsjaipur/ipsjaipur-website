...
const BASE = '/dashboard/page-content/home';

const SECTIONS = [
  {
    key: 'banner',
    label: 'Hero Banner',
    description: 'Slider images shown at the top of the home page',
    icon: ImageIcon,
    color: '#3B82F6',
  },
  {
    ...
    description: 'AICTE, RTU and other approval logos and labels',
    icon: CheckCircle2,
    color: '#10B981',
  },
  {
    key: 'methodology',
    label: 'About / IPS Methodology',
    description: 'Heading, description paragraph and feature list items',
    icon: Info,
    color: '#F59E0B',
  },
  {
    key: 'pathway',
    label: 'Pathway to Excellence',
    description: '10 feature cards around the graduate centre image',
    icon: LayoutGrid,
    color: '#8B5CF6',
  },
  {
    ...
    description: 'MBA, BBA, BCA program cards with features and links',
    icon: BookOpen,
    color: '#EC4899',
  },
  {
    key: 'placements',
    label: 'Placements',
    description: 'Stats, company logos marquee, and placement student images',
    icon: TrendingUp,
    color: '#EF4444',
  },
  {
    key: 'videoGallery',
    label: 'Video Gallery',
    description: 'Instagram reel thumbnails and URLs',
    icon: Video,
    color: '#F97316',
  },
  {
    key: 'achievers',
    label: 'Our Achievers',
    description: 'Achiever images shown in the slider',
    icon: Award,
    color: '#EAB308',
  },
  {
    ...
    description: 'Heading, description, and testimonial slide images',
    icon: Users,
    color: '#06B6D4',
  },
];
...
export default function HomePageSectionsPage() {
  ...
  async function handleSeed(force = false) {
    if (
      force &&
      !window.confirm('This will OVERWRITE all home page sections with the original static defaults. Are you sure?')
    )
      return;

    setSeeding(true);
    try {
      const res = await fetch(`/api/home-content/seed${force ? '?force=true' : ''}`, {
        method: 'POST',
        credentials: 'include',
      });
      const data = await res.json();
  ...
  return (
    <div className="max-w-[900px] mx-auto">
      ...
      <div className="flex items-center gap-2 text-[12px] text-[#77838f] mb-4">
        ...
        <span className="text-[#222] font-medium">Home Page</span>
      </div>
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
        <div className="flex items-center gap-3">
          <div className="w-10 h-10 bg-[#eb5905]/10 rounded-xl flex items-center justify-center">
            <Home className="w-5 h-5 text-[#eb5905]" />
          </div>
          <div>
            <h1 className="text-[20px] font-bold text-[#222]">Home Page</h1>
      ...
      {/* Warning banner when DB is not seeded */}
      {!loading && anyEmpty && (
        <div className="flex items-start gap-3 bg-amber-50 border border-amber-200 rounded-xl p-4 mb-6">
          ...
          <div>
            ...
            <p className="text-[12px] text-amber-700 mt-0.5">
              Click <strong>Seed Missing</strong> to populate them with the current live content. Safe — won&apos;t
