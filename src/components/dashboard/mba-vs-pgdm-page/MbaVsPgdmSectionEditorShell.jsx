...
/**
 * Shell that handles fetching, saving, and layout for a single MBA vs PGDM page section editor.
 *
 * Props:
 *  - sectionKey: 'banner' | 'content' | 'comparison'
 *  - title: string
 *  - description: string
 *  - children: (data, setData, saving) => ReactNode  — render prop
 */
export default function MbaVsPgdmSectionEditorShell({ sectionKey, title, description, children }) {
  ...
  const load = useCallback(async () => {
    ...
    setError(null);
    try {
      const res  = await fetch(`/api/mba-vs-pgdm-content?section=${sectionKey}`, {
        credentials: 'include',
      });
  ...
  async function handleSave() {
    ...
    setSaving(true);
    try {
      const res  = await fetch('/api/mba-vs-pgdm-content', {
        method:      'PUT',
        credentials: 'include',
  ...
  return (
    <div className="max-w-[860px] mx-auto">
      ...
      <div className="flex items-center gap-2 text-[12px] text-[#77838f] mb-4 flex-wrap">
        ...
        <Link href="/dashboard/page-content/mba-vs-pgdm" className="hover:text-[#eb5905] transition">
          MBA vs PGDM Page
        </Link>
        <ChevronRight className="w-3.5 h-3.5 shrink-0" />
