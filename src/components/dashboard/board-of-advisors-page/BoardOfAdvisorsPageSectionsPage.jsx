...
const BASE = '/dashboard/page-content/board-of-advisors';
...
const SECTIONS = [
  ...
  },
  {
    key: 'advisors',
    label: 'Advisors',
    description: 'Section heading, subtitle, and all advisor profile cards',
    icon: Users,
    color: '#eb5905',
  },
  {
    key: 'bridging',
    label: 'Bridging Theory CTA',
    description: 'Bottom CTA section — heading, paragraph, and button',
    icon: ArrowRight,
    color: '#10B981',
  },
];

export default function BoardOfAdvisorsPageSectionsPage() {
  ...
  async function fetchStatus() {
    setLoading(true);
    try {
      const res = await fetch('/api/board-of-advisors-content', { credentials: 'include' });
  ...
  async function handleSeed(force = false) {
    if (
      force &&
      !window.confirm(
        'This will OVERWRITE all Board of Advisors page sections with the original static defaults. Are you sure?',
      )
    )
      return;
    ...
    setSeeding(true);
    try {
      const res = await fetch(`/api/board-of-advisors-content/seed${force ? '?force=true' : ''}`, {
        method: 'POST',
        credentials: 'include',
      });
      const data = await res.json();
  ...
  return (
    <div className="max-w-[900px] mx-auto">
      ...
      <div className="flex items-center gap-2 text-[12px] text-[#77838f] mb-4">
        ...
        <span className="text-[#222] font-medium">Board of Advisors Page</span>
      </div>
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
        <div className="flex items-center gap-3">
          <div className="w-10 h-10 bg-[#eb5905]/10 rounded-xl flex items-center justify-center">
            <UserCheck className="w-5 h-5 text-[#eb5905]" />
          </div>
          <div>
            <h1 className="text-[20px] font-bold text-[#222]">Board of Advisors Page</h1>
