...
/**
 * Shell that handles fetching, saving, and layout for a single Board of Advisors section editor.
 *
 * Props:
 *  - sectionKey: 'banner' | 'advisors' | 'bridging'
 *  - title: string
 *  - description: string
 *  - children: (data, setData, saving) => ReactNode
 */
export default function BoardOfAdvisorsSectionEditorShell({
  sectionKey,
  title,
  description,
  children,
  ...
  const load = useCallback(async () => {
    ...
    setError(null);
    try {
      const res  = await fetch(`/api/board-of-advisors-content?section=${sectionKey}`, {
        credentials: 'include',
      });
  ...
  async function handleSave() {
    ...
    setSaving(true);
    try {
      const res  = await fetch('/api/board-of-advisors-content', {
        method:      'PUT',
        credentials: 'include',
  ...
  return (
    <div className="max-w-[860px] mx-auto">
      ...
      <div className="flex items-center gap-2 text-[12px] text-[#77838f] mb-4 flex-wrap">
        ...
        <Link href="/dashboard/page-content/board-of-advisors" className="hover:text-[#eb5905] transition">
