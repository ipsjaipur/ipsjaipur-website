...
/**
 * Shell that handles fetching, saving, and layout for a single Faculty section editor.
 *
 * Props:
 *  - sectionKey: 'banner' | 'faculty' | 'mentors'
 *  - title: string
 *  - description: string
 *  - children: (data, setData, saving) => ReactNode
 */
export default function FacultySectionEditorShell({ sectionKey, title, description, children }) {
  ...
  const load = useCallback(async () => {
    ...
    setError(null);
    try {
      const res  = await fetch(`/api/faculty-content?section=${sectionKey}`, {
        credentials: 'include',
      });
  ...
  return (
    <div className="max-w-[860px] mx-auto">
      ...
      <div className="flex items-center gap-2 text-[12px] text-[#77838f] mb-4 flex-wrap">
        ...
        <Link href="/dashboard/page-content/faculty" className="hover:text-[#eb5905] transition">
          Faculty Page
        </Link>
        <ChevronRight className="w-3.5 h-3.5 shrink-0" />
