...
/**
 * Shell that handles fetching, saving, and layout for a single Contact page section editor.
 *
 * Props:
 *  - sectionKey: 'banner' | 'info'
 *  - title: string
 *  - description: string
 *  - children: (data, setData, saving) => ReactNode
 */
export default function ContactSectionEditorShell({ sectionKey, title, description, children }) {
  ...
  const load = useCallback(async () => {
    ...
    setError(null);
    try {
      const res  = await fetch(`/api/contact-content?section=${sectionKey}`, {
        credentials: 'include',
      });
  ...
  return (
    <div className="max-w-[860px] mx-auto">
      ...
      <div className="flex items-center gap-2 text-[12px] text-[#77838f] mb-4 flex-wrap">
        ...
        <Link href="/dashboard/page-content/contact" className="hover:text-[#eb5905] transition">
          Contact Page
        </Link>
        <ChevronRight className="w-3.5 h-3.5 shrink-0" />
