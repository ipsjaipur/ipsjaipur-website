...
export default function MBAFeeEditor() {
  return (
    <MBASectionEditorShell
      sectionKey="feeStructure"
      title="Fee Structure"
        ...
        return (
          <div className="bg-white rounded-2xl border border-[#e2e8f0] p-5 space-y-4">
            ...
            <div>
              ...
              <input
                type="text"
                value={data?.feeHeading || ''}
                onChange={(e) => set('feeHeading')(e.target.value)}
                placeholder="Fee Structure of MBA"
                className={cls}
              />
            </div>
            ...
            <div>
              <label className="block text-[11px] font-semibold text-[#4a5568] uppercase tracking-wide mb-1">
                Scholarship Text (English)
              </label>
              <textarea
                rows={3}
                value={data?.feeEnglishText || ''}
                onChange={(e) => set('feeEnglishText')(e.target.value)}
                placeholder="Scholarship is Available on the basis of Academic Performance…"
            ...
            <div>
              <label className="block text-[11px] font-semibold text-[#4a5568] uppercase tracking-wide mb-1">
                Scholarship Text (Hindi)
              </label>
              <textarea
                rows={3}
                value={data?.feeHindiText || ''}
                onChange={(e) => set('feeHindiText')(e.target.value)}
                placeholder="छात्रवृत्ति शैक्षणिक प्रदर्शन एवं साइको मैट्रिक्स टेस्ट के आधार पर…"
                className={`${cls} resize-none`}
              />
            </div>
