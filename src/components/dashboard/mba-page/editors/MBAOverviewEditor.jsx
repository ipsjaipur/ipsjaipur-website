...
// ── Why IPS points list item ──────────────────────────────────────────────────
function WhyIpsPointItem(item, index, update) {
  return (
    <div>
      <label className="block text-[11px] font-semibold text-[#4a5568] uppercase tracking-wide mb-1">
        Point (HTML supported for bold: &lt;b&gt;…&lt;/b&gt;)
      </label>
      <textarea
        rows={4}
        value={item.value || ''}
...
function DescParaItem(item, index, update) {
  return (
    <div>
      <label className="block text-[11px] font-semibold text-[#4a5568] uppercase tracking-wide mb-1">
        Paragraph (HTML supported for bold)
      </label>
      <textarea
        rows={3}
        value={item.value || ''}
...
export default function MBAOverviewEditor() {
  return (
    <MBASectionEditorShell
      sectionKey="overview"
      title="Overview Section"
      description="Header, highlights (with bilingual text & icon), description paragraphs, and Why IPS sub-section."
    >
      {(data, setData) => {
          ...
          <>
            ...
            <div className="bg-white rounded-2xl border border-[#e2e8f0] p-5 space-y-4">
              ...
              <Field
                ...
                placeholder="MASTER IN BUSINESS ADMINISTRATION (MBA)"
              />
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <ImageUpload
                  label="Approval Strip Image (blck.webp)"
            ...
            <div className="bg-white rounded-2xl border border-[#e2e8f0] p-5">
              ...
              <div className="mb-4">
                <Field
                  ...
                  placeholder="MBA HIGHLIGHTS:"
                />
              </div>
              <p className="text-[12px] text-[#77838f] mb-4">
                Each highlight shows an icon, English text and optional Hindi translation. Click an icon button in the
                grid to select it — a live preview updates instantly.
              </p>
              <SortableList
            ...
            <div className="bg-white rounded-2xl border border-[#e2e8f0] p-5 space-y-4">
              ...
              <div>
                <p className="text-[11px] font-semibold text-[#4a5568] uppercase tracking-wide mb-2">
                  Bullet Points (HTML for bold: &lt;b&gt;Lead:&lt;/b&gt; text)
                </p>
                <SortableList
                  items={whyPoints}
