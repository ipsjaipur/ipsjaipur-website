...
export default function MBASpecializationsEditor() {
  return (
    <MBASectionEditorShell
      sectionKey="specializations"
      title="Specializations & Eligibility"
        ...
        return (
          <>
            ...
            <div className="bg-white rounded-2xl border border-[#e2e8f0] p-5">
              <div className="mb-4">
                <Field
                  label="Specializations Heading"
                  value={data?.specializationsHeading}
                  onChange={set('specializationsHeading')}
                  placeholder="SPECIALIZATIONS ARE AVAILABLE IN FOLLOWING AREA:"
                />
              </div>
            ...
            <div className="bg-white rounded-2xl border border-[#e2e8f0] p-5">
              <div className="mb-4">
                <Field
                  ...
                  value={data?.eligibilityHeading}
                  onChange={set('eligibilityHeading')}
              ...
              <SortableList
                items={data?.eligibilityPoints || []}
                onChange={(eligibilityPoints) => setData({ ...data, eligibilityPoints })}
                renderItem={EligibilityPointItem}
                onAdd={() => ({ text: '', order: 0 })}
                addLabel="Add Eligibility Point"
              />
            </div>
            ...
            <div className="bg-white rounded-2xl border border-[#e2e8f0] p-5">
              ...
              <p className="text-[12px] text-[#77838f] mb-4">
                Shown indented below the eligibility points — used when candidates have equal scores.
              </p>
              <SortableList
                items={data?.eligibilityPriorities || []}
                onChange={(eligibilityPriorities) => setData({ ...data, eligibilityPriorities })}
                renderItem={PriorityItem}
                onAdd={() => ({ text: '', order: 0 })}
                addLabel="Add Priority Rule"
              />
            </div>
          </>
        );
