...
const BASE = '/dashboard/page-content/mba';

const SECTIONS = [
  {
    key: 'banner',
    label: 'Banner Slides',
    description: 'Slider images shown at the top of the MBA page',
    icon: ImageIcon,
    color: '#3B82F6',
  },
  {
    key: 'overview',
    label: 'Overview',
    description: 'Degree title, highlights (bilingual), description paragraphs, Why IPS section',
    icon: BookOpen,
    color: '#F59E0B',
  },
  {
    key: 'syllabus',
    label: 'Syllabus',
    description: 'PDF download links for each semester syllabus',
    icon: FileText,
    color: '#10B981',
  },
  {
    key: 'specializations',
    label: 'Specializations & Eligibility',
    description: 'Available specializations and eligibility criteria points',
    icon: ListOrdered,
    color: '#8B5CF6',
  },
  {
    ...
    description: 'How to apply — online and offline steps with links',
    icon: ScrollText,
    color: '#EC4899',
  },
  {
    ...
    description: 'List of documents needed for MBA admission',
    icon: ClipboardList,
    color: '#EF4444',
  },
  {
    key: 'selectionProcedure',
    label: 'Selection Procedure',
    description: 'Step-by-step selection process (Step 1 → Step 5)',
    icon: BadgeCheck,
    color: '#06B6D4',
  },
  {
    key: 'feeStructure',
    label: 'Fee Structure',
    description: 'Fee section heading and scholarship contact text (English + Hindi)',
    icon: DollarSign,
    color: '#EAB308',
  },
  {
  ...
  },
  {
    ...
    description: 'YouTube embed videos in the right sidebar',
    icon: Sidebar,
    color: '#6366F1',
  },
];

export default function MBAPageSectionsPage() {
  ...
  async function fetchStatus() {
    setLoading(true);
    try {
      const res = await fetch('/api/mba-content', { credentials: 'include' });
  ...
  async function handleSeed(force = false) {
    if (
      force &&
      !window.confirm('This will OVERWRITE all MBA page sections with the original static defaults. Are you sure?')
    )
      return;

    setSeeding(true);
    try {
      const res = await fetch(`/api/mba-content/seed${force ? '?force=true' : ''}`, {
        method: 'POST',
        credentials: 'include',
      });
      const data = await res.json();
  ...
  return (
    <div className="max-w-[900px] mx-auto">
      ...
      <div className="flex items-center gap-2 text-[12px] text-[#77838f] mb-4">
        ...
        <span className="text-[#222] font-medium">MBA Page</span>
      </div>
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-[20px] font-bold text-[#222]">MBA Page</h1>
