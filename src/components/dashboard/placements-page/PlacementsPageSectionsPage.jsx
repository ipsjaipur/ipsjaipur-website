...
const BASE = '/dashboard/page-content/placements';

const SECTIONS = [
  {
    key: 'banner',
    label: 'Page Banner',
    description: 'Hero banner image, page title, and image focal point',
    icon: ImageIcon,
    color: '#06B6D4',
  },
  {
    key: 'stats',
    label: 'Placement Stats (Sector-wise)',
    description: 'Infographic images showing placement, MBA internship, and live project sector data',
    icon: BarChart2,
    color: '#3B82F6',
  },
  {
    ...
    description: 'Contact name, phone numbers and emails shown in the marquee strip',
    icon: Phone,
    color: '#10B981',
  },
  {
    ...
    description: 'Grid of placed student cards — photo, name, and company for each student',
    icon: Users,
    color: '#EF4444',
  },
  {
    key: 'resumeBook',
    label: 'Resume Book',
    description: 'YouTube video-résumé grid with heading, description and CTA link',
    icon: BookOpen,
    color: '#F59E0B',
  },
  {
    key: 'faq',
    label: 'Industry Hiring Practices (FAQ)',
    description: 'Collapsible accordion with industry categories and descriptive answer text',
    icon: HelpCircle,
    color: '#8B5CF6',
  },
];

export default function PlacementsPageSectionsPage() {
  ...
  async function fetchStatus() {
    setLoading(true);
    try {
      const res = await fetch('/api/placements-content', { credentials: 'include' });
  ...
  async function handleSeed(force = false) {
    if (
      force &&
      !window.confirm(
        'This will OVERWRITE all placements page sections with the original static defaults. Are you sure?',
      )
    )
      return;
    ...
    setSeeding(true);
    try {
      const res = await fetch(`/api/placements-content/seed${force ? '?force=true' : ''}`, {
        method: 'POST',
        credentials: 'include',
      });
      const data = await res.json();
  ...
  return (
    <div className="max-w-[900px] mx-auto">
      ...
      <div className="flex items-center gap-2 text-[12px] text-[#77838f] mb-4">
        ...
        <span className="text-[#222] font-medium">Placements Page</span>
      </div>
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
        <div className="flex items-center gap-3">
          <div className="w-10 h-10 bg-[#eb5905]/10 rounded-xl flex items-center justify-center">
            <TrendingUp className="w-5 h-5 text-[#eb5905]" />
          </div>
          <div>
            <h1 className="text-[20px] font-bold text-[#222]">Placements Page</h1>
            <p className="text-[12px] text-[#77838f]">Edit each section. Changes save per section.</p>
          </div>
        </div>
      ...
      <div className="flex items-start gap-3 bg-blue-50 border border-blue-200 rounded-xl p-4 mb-6">
        ...
        <p className="text-[12px] text-blue-700">
          <strong>Placement slider images</strong> and <strong>company logos / partner marquee</strong> are shared with
          the Home page and managed under{' '}
          <Link href="/dashboard/page-content/home?section=placements" className="underline font-semibold">
            Home Page → Placements
          </Link>
          . Changes there also update this page automatically.
        </p>
      </div>
        ...
        <div className="flex items-start gap-3 bg-amber-50 border border-amber-200 rounded-xl p-4 mb-6">
          ...
          <div>
            ...
            <p className="text-[12px] text-amber-700 mt-0.5">
              Click <strong>Seed Missing</strong> to populate them with the current live content.
            </p>
          </div>
        </div>
      )}
