 ...
 * Mission & Vision page section editor.
 *
 * Props:
 *  - sectionKey : 'banner' | 'institutional' | 'vision_mission' | 'core_values'
 *  - title      : string
 *  - description: string
 *  - children   : (data, setData, saving) => ReactNode
 */
export default function MissionVisionSectionEditorShell({
  sectionKey,
  title,
  description,
  children,
  ...
  const load = useCallback(async () => {
    ...
    setError(null);
    try {
      const res  = await fetch(`/api/mission-vision-content?section=${sectionKey}`, {
        credentials: 'include',
      });
  ...
  async function handleSave() {
    ...
    setSaving(true);
    try {
      const res  = await fetch('/api/mission-vision-content', {
        method:      'PUT',
        credentials: 'include',
  ...
  return (
    <div className="max-w-[860px] mx-auto">
      ...
      <div className="flex items-center gap-2 text-[12px] text-[#77838f] mb-4 flex-wrap">
        ...
        <Link href="/dashboard/page-content/mission-vision" className="hover:text-[#eb5905] transition">
          Mission &amp; Vision Page
        </Link>
        <ChevronRight className="w-3.5 h-3.5 shrink-0" />
