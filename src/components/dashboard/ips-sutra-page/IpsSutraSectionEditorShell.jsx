...
/**
 * Shell that handles fetching, saving, and layout for a single IPS Sutra section editor.
 *
 * Props:
 *  - sectionKey: string  e.g. 'content' | 'advantages'
 *  - title: string
 *  - description: string
 *  - children: (data, setData, saving) => ReactNode
 */
export default function IpsSutraSectionEditorShell({ sectionKey, title, description, children }) {
  ...
  const load = useCallback(async () => {
    ...
    setError(null);
    try {
      const res  = await fetch(`/api/ips-sutra-content?section=${sectionKey}`, { credentials: 'include' });
  ...
  async function handleSave() {
    ...
    setSaving(true);
    try {
      const res  = await fetch('/api/ips-sutra-content', {
        method: 'PUT',
        credentials: 'include',
  ...
  return (
    <div className="max-w-[860px] mx-auto">
      ...
      <div className="flex items-center gap-2 text-[12px] text-[#77838f] mb-4 flex-wrap">
        ...
        <Link href="/dashboard/page-content/ips-sutra" className="hover:text-[#eb5905] transition">
          IPS Sutra Page
        </Link>
        <ChevronRight className="w-3.5 h-3.5 shrink-0" />
