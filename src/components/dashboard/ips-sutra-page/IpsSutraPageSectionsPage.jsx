...
const BASE = '/dashboard/page-content/ips-sutra';

const SECTIONS = [
  {
    key: 'content',
    label: 'Page Content',
    description: 'Banner title & image, main heading, Hindi paragraphs, CTA button, side image',
    icon: FileText,
    color: '#F59E0B',
  },
  {
    key: 'advantages',
    label: 'IPS Advantages',
    description: 'Section heading, Apply Now button, advantage cards (icon + text)',
    icon: Star,
    color: '#eb5905',
  },
];

export default function IpsSutraPageSectionsPage() {
  ...
  async function fetchStatus() {
    setLoading(true);
    try {
      const res  = await fetch('/api/ips-sutra-content', { credentials: 'include' });
  ...
  async function handleSeed(force = false) {
    if (
      force &&
      !window.confirm(
        'This will OVERWRITE all IPS Sutra page sections with the original static defaults. Are you sure?'
      )
    )
      return;
    setSeeding(true);
    try {
      const res  = await fetch(
        `/api/ips-sutra-content/seed${force ? '?force=true' : ''}`,
  ...
  return (
    <div className="max-w-[900px] mx-auto">
      ...
      <div className="flex items-center gap-2 text-[12px] text-[#77838f] mb-4">
        ...
        <span className="text-[#222] font-medium">IPS Sutra Page</span>
      </div>
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
        <div className="flex items-center gap-3">
          <div className="w-10 h-10 bg-[#eb5905]/10 rounded-xl flex items-center justify-center">
            <FileText className="w-5 h-5 text-[#eb5905]" />
          </div>
          <div>
            <h1 className="text-[20px] font-bold text-[#222]">IPS Sutra Page</h1>
        ...
        <div className="flex items-start gap-3 bg-amber-50 border border-amber-200 rounded-xl p-4 mb-6">
          ...
          <div>
            ...
            <p className="text-[12px] text-amber-700 mt-0.5">
              Click <strong>Seed Missing</strong> to populate them with the current static content. Safe
