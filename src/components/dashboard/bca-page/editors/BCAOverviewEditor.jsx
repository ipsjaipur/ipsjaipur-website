...
const ICON_OPTIONS = [
  ...
  { name: 'Cloud',         Icon: Cloud,         label: 'Cloud Computing' },
];
...
function HighlightItem(item, index, update) {
  return (
    <div className="space-y-3">
      ...
      <div>
        <label className="block text-[11px] font-semibold text-[#4a5568] uppercase tracking-wide mb-1">English Text</label>
      ...
      <div>
        <label className="block text-[11px] font-semibold text-[#4a5568] uppercase tracking-wide mb-1">Hindi Text</label>
        <textarea rows={2} value={item.hindiText || ''} onChange={(e) => update({ hindiText: e.target.value })}
...
function DescParaItem(item, index, update) {
  return (
    <div>
      ...
      <textarea rows={3} value={item.value || ''} onChange={(e) => update({ value: e.target.value })}
...
function WhyIpsPointItem(item, index, update) {
  return (
    <div>
      ...
      <textarea rows={4} value={item.value || ''} onChange={(e) => update({ value: e.target.value })}
...
export default function BCAOverviewEditor() {
  return (
        ...
        return (
          <>
            ...
            <div className="bg-white rounded-2xl border border-[#e2e8f0] p-5 space-y-4">
              ...
              <Field label="Degree Title" value={data?.overviewTitle} onChange={set('overviewTitle')}
                placeholder="Bachelor of Computer Applications (BCA)" />
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <Field label="Duration" value={data?.overviewDuration} onChange={set('overviewDuration')}
                  placeholder="3 Years" />
                <Field label="Caption below title" value={data?.overviewCaption} onChange={set('overviewCaption')} />
              </div>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <ImageUpload label="RTU Logo" value={data?.rtuLogoUrl || ''} onChange={set('rtuLogoUrl')}
                  aspectHint="Square logo" maxWidth="200px" previewHeight="100px" objectFit="contain" />
                <ImageUpload label="AICTE Logo" value={data?.aicteLogoUrl || ''} onChange={set('aicteLogoUrl')}
                  aspectHint="Square logo" maxWidth="200px" previewHeight="100px" objectFit="contain" />
              </div>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <ImageUpload label="Approval Strip Image" value={data?.approvalImageUrl || ''}
                  onChange={set('approvalImageUrl')} aspectHint="Approval logos strip"
                  maxWidth="240px" previewHeight="100px" objectFit="contain" />
                <Field label="Approval Caption" value={data?.approvalImageCaption}
            ...
            <div className="bg-white rounded-2xl border border-[#e2e8f0] p-5">
              ...
              <div className="mb-4">
                <Field label="Highlights Section Heading" value={data?.highlightsHeading}
                  onChange={set('highlightsHeading')} placeholder="BCA HIGHLIGHTS:" />
              </div>
              <SortableList
            ...
            <div className="bg-white rounded-2xl border border-[#e2e8f0] p-5 space-y-4">
              ...
              <Field label="Intro Paragraph" value={data?.whyIpsIntro} onChange={set('whyIpsIntro')}
                multiline rows={3} />
              <div>
