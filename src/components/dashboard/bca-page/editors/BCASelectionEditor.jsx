...
export default function BCASelectionEditor() {
  return (
    <BCASectionEditorShell sectionKey="selectionProcedure" title="Selection Procedure"
      description="Step-by-step selection process shown on the BCA page.">
      {(data, setData) => (
        <div className="bg-white rounded-2xl border border-[#e2e8f0] p-5">
          <div className="mb-4">
            <label className="block text-[11px] font-semibold text-[#4a5568] uppercase tracking-wide mb-1">Section Heading</label>
            <input type="text" value={data?.selectionHeading || ''}
              onChange={(e) => setData({ ...data, selectionHeading: e.target.value })}
          ...
          <SortableList items={data?.selectionSteps || []}
            onChange={(selectionSteps) => setData({ ...data, selectionSteps })}
