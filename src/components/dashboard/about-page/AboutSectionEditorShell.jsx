...
/**
 * Shell that handles fetching, saving, and layout for a single About page section editor.
 *
 * Props:
 *  - sectionKey: string  e.g. 'content' | 'sidebar'
 *  - title: string
 *  - description: string
 *  - children: (data, setData, saving) => ReactNode  — render prop
 */
export default function AboutSectionEditorShell({ sectionKey, title, description, children }) {
  ...
  const load = useCallback(async () => {
    ...
    setError(null);
    try {
      const res  = await fetch(`/api/about-content?section=${sectionKey}`, { credentials: 'include' });
  ...
  return (
    <div className="max-w-[860px] mx-auto">
      ...
      <div className="flex items-center gap-2 text-[12px] text-[#77838f] mb-4 flex-wrap">
        ...
        <Link href="/dashboard/page-content/about" className="hover:text-[#eb5905] transition">
          About Page
        </Link>
        <ChevronRight className="w-3.5 h-3.5 shrink-0" />
