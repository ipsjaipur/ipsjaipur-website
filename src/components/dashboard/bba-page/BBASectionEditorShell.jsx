...
/**
 * Shell that handles fetching, saving, and layout for a single BBA section editor.
 *
 * Props:
 *  - sectionKey: string  e.g. 'overview', 'faq'
 *  - title: string
 *  - description: string
 *  - children: (data, setData, saving) => ReactNode
 */
export default function BBASectionEditorShell({ sectionKey, title, description, children }) {
  ...
  const load = useCallback(async () => {
    ...
    setError(null);
    try {
      const res  = await fetch(`/api/bba-content?section=${sectionKey}`, { credentials: 'include' });
  ...
  async function handleSave() {
    ...
    setSaving(true);
    try {
      const res  = await fetch('/api/bba-content', {
        method: 'PUT',
        credentials: 'include',
  ...
  return (
    <div className="max-w-[860px] mx-auto">
      ...
      <div className="flex items-center gap-2 text-[12px] text-[#77838f] mb-4 flex-wrap">
        ...
        <Link href="/dashboard/page-content/bba" className="hover:text-[#eb5905] transition">BBA Page</Link>
        <ChevronRight className="w-3.5 h-3.5 shrink-0" />
      ...
      {loading ? (
        ...
        <div className="space-y-5">{children(data, setData, saving)}</div>
      )}
      ...
      {data && !loading && (
        <div className="sticky bottom-4 mt-6 flex justify-end">
          <button type="button" onClick={handleSave} disabled={saving}
            className="inline-flex items-center gap-2 bg-[#eb5905] text-white text-[13px] font-semibold px-6 py-3 rounded-xl shadow-lg hover:bg-[#c94f05] transition disabled:opacity-60 cursor-pointer disabled:cursor-not-allowed">
            {saving ? <Loader2 className="w-4 h-4 animate-spin" /> : <Save className="w-4 h-4" />}
            {saving ? 'Saving…' : 'Save Changes'}
          </button>
        </div>
