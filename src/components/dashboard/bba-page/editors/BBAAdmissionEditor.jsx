...
export default function BBAAdmissionEditor() {
  return (
    <BBASectionEditorShell sectionKey="admissionProcess" title="Admission Process"
        ...
        <>
          <div className="bg-white rounded-2xl border border-[#e2e8f0] p-5 space-y-4">
            ...
            <div>
              ...
              <input type="text" value={data?.admissionHeading || ''} onChange={(e) => setData({ ...data, admissionHeading: e.target.value })}
                placeholder="How to Apply:"
                className="w-full border border-[#e2e8f0] rounded-lg px-3 py-2 text-[13px] focus:outline-none focus:border-[#eb5905] transition" />
            </div>
            <div>
              <label className="block text-[11px] font-semibold text-[#4a5568] uppercase tracking-wide mb-1">Intro Text</label>
              <textarea rows={2} value={data?.admissionIntro || ''} onChange={(e) => setData({ ...data, admissionIntro: e.target.value })}
          ...
          <div className="bg-white rounded-2xl border border-[#e2e8f0] p-5">
            ...
            <SortableList items={data?.admissionSteps || []} onChange={(admissionSteps) => setData({ ...data, admissionSteps })}
              renderItem={AdmissionStepItem} onAdd={() => ({ label: '', description: '', linkText: '', linkHref: '', order: 0 })}
              addLabel="Add Step" />
          </div>
        </>
      )}
