  ...
  return (
    <div className="space-y-2">
      ...
      <div className="flex items-center gap-2">
        <input type="checkbox" id={`bba-doc-bold-${index}`} checked={!!item.isBold}
          onChange={(e) => update({ isBold: e.target.checked })} className="w-4 h-4 accent-[#eb5905]" />
        <label htmlFor={`bba-doc-bold-${index}`} className="text-[13px] font-medium text-[#4a5568]">Show as bold text</label>
      </div>
    </div>
  );
}

export default function BBADocumentsEditor() {
  return (
    <BBASectionEditorShell sectionKey="documentsRequired" title="Documents Required"
      description="List of documents students need to submit for BBA admission.">
      {(data, setData) => (
        <div className="bg-white rounded-2xl border border-[#e2e8f0] p-5">
          <div className="mb-4">
            <label className="block text-[11px] font-semibold text-[#4a5568] uppercase tracking-wide mb-1">Section Heading</label>
            <input type="text" value={data?.documentsHeading || ''} onChange={(e) => setData({ ...data, documentsHeading: e.target.value })}
          ...
          <SortableList items={data?.documents || []} onChange={(documents) => setData({ ...data, documents })}
            renderItem={DocumentItem} onAdd={() => ({ text: '', isBold: false, order: 0 })} addLabel="Add Document" />
        </div>
      )}
