...
export default function BBASpecializationsEditor() {
  return (
    <BBASectionEditorShell sectionKey="specializations" title="Specializations & Eligibility" description="Manage available specializations and eligibility criteria.">
      {(data, setData) => {
        ...
        return (
          <>
            <div className="bg-white rounded-2xl border border-[#e2e8f0] p-5">
              <div className="mb-4">
                <Field label="Specializations Heading" value={data?.specializationsHeading} onChange={set('specializationsHeading')} placeholder="SPECIALIZATIONS AVAILABLE:" />
              </div>
              ...
              <SortableList items={data?.specializations || []} onChange={(specializations) => setData({ ...data, specializations })}
                renderItem={SpecItem} onAdd={() => ({ name: '', order: 0 })} addLabel="Add Specialization" />
            </div>

            <div className="bg-white rounded-2xl border border-[#e2e8f0] p-5">
              <div className="mb-4">
                <Field label="Eligibility Heading" value={data?.eligibilityHeading} onChange={set('eligibilityHeading')} placeholder="Eligibility:" />
              </div>
              ...
              <SortableList items={data?.eligibilityPoints || []} onChange={(eligibilityPoints) => setData({ ...data, eligibilityPoints })}
            ...
            <div className="bg-white rounded-2xl border border-[#e2e8f0] p-5">
              ...
              <SortableList items={data?.eligibilityPriorities || []} onChange={(eligibilityPriorities) => setData({ ...data, eligibilityPriorities })}
