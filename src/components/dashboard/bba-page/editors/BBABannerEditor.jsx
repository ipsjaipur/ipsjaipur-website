    ...
    <div className="space-y-3">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
        ...
        <div className="flex items-center gap-2 pt-5">
          <input type="checkbox" id={`bba-priority-${index}`} checked={!!item.priority}
            onChange={(e) => update({ priority: e.target.checked })} className="w-4 h-4 accent-[#eb5905]" />
          <label htmlFor={`bba-priority-${index}`} className="text-[13px] font-medium text-[#4a5568]">Priority load (first slide)</label>
        </div>
      </div>
    </div>
  );
...
export default function BBABannerEditor() {
  return (
    <BBASectionEditorShell sectionKey="banner" title="Banner Slides" description="Manage the slider images shown at the top of the BBA page.">
      {(data, setData) => (
