...
/**
 * Shell that handles fetching, saving, and layout for a single Student Life page section editor.
 *
 * Props:
 *  - sectionKey: string  e.g. 'banner' | 'campus' | 'student-club' | etc.
 *  - title: string
 *  - description: string
 *  - children: (data, setData, saving) => ReactNode  — render prop
 */
export default function StudentLifeSectionEditorShell({ sectionKey, title, description, children }) {
  ...
  const load = useCallback(async () => {
    ...
    setError(null);
    try {
      const res  = await fetch(`/api/student-life-content?section=${sectionKey}`, { credentials: 'include' });
  ...
  async function handleSave() {
    ...
    setSaving(true);
    try {
      const res  = await fetch('/api/student-life-content', {
        method:      'PUT',
        credentials: 'include',
  ...
  return (
    <div className="max-w-[860px] mx-auto">
      ...
      <div className="flex items-center gap-2 text-[12px] text-[#77838f] mb-4 flex-wrap">
        ...
        <Link href="/dashboard/page-content/student-life" className="hover:text-[#eb5905] transition">
          Student Life Page
        </Link>
        <ChevronRight className="w-3.5 h-3.5 shrink-0" />
