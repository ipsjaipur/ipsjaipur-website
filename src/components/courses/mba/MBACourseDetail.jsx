...
// ── Icon registry — maps DB string name → Lucide component ────────────────────
const ICON_MAP = {
  CheckCircle2,
  School,
  GraduationCap,
...
export default function MBACourseDetail({ mbaContent = {} }) {
  const [openFAQ, setOpenFAQ] = useState(0);

  const imgBase = process.env.NEXT_PUBLIC_IMG_PATH || '';

  // ── Pull raw data from each section (no fallbacks) ───────────────────────
  const overviewData = mbaContent?.overview || {};
  const syllabusData = mbaContent?.syllabus || {};
  const specsData = mbaContent?.specializations || {};
  const admissionData = mbaContent?.admissionProcess || {};
  const docsData = mbaContent?.documentsRequired || {};
  const selectionData = mbaContent?.selectionProcedure || {};
  const feeData = mbaContent?.feeStructure || {};
  const faqData = mbaContent?.faq || {};
  const sidebarData = mbaContent?.sidebar || {};
  ...
  // Has enough overview content to show the section?
  ...
  const eligibilityPriorities =
    specsData.eligibilityPriorities?.length > 0
      ...
      : [];
  const hasSpecializations = specializations.length > 0 || eligibilityPoints.length > 0;
              ...
              <motion.div initial={false} animate={{ opacity: 1 }} transition={{ duration: 0.5 }}>
                ...
                {hasOverview && (
                  <div className="mb-8" id="overview">
                    {/* Header: RTU logo | Title | AICTE logo */}
                    ...
                    {descriptionParagraphs.length > 0 && (
                      <div className="space-y-4 mb-8">
                        {descriptionParagraphs.map((para, i) => (
                          <p
                            key={i}
                            className="text-[#444444] lg:text-[16px] text-[14px] font-rubik leading-[1.8]"
                  ...
                  <div id="syllabus" className="mb-8">
                    ...
                    <div className="space-y-2">
                      {syllabusItems.map((item, index) => {
                        const href = item.pdfFile?.startsWith('http')
                          ? item.pdfFile
                ...
                {hasSpecializations && (
                  <div id="specializations" className="mb-8">
                    {specsHeading && (
                      <h4 className="text-lg font-bold text-gray-800 mb-4 font-rubik">{specsHeading}</h4>
                    )}
