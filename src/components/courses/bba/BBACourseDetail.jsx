...
export default function BBACourseDetail({ bbaContent = {} }) {
  const [openFAQ, setOpenFAQ] = useState(0);

  const imgBase = process.env.NEXT_PUBLIC_IMG_PATH || '';

  // ── Pull raw data — no fallbacks ─────────────────────────────────────────
  const overviewData = bbaContent?.overview || {};
  const syllabusData = bbaContent?.syllabus || {};
  const specsData = bbaContent?.specializations || {};
  const admissionData = bbaContent?.admissionProcess || {};
  const docsData = bbaContent?.documentsRequired || {};
  const selectionData = bbaContent?.selectionProcedure || {};
  const feeData = bbaContent?.feeStructure || {};
  const faqData = bbaContent?.faq || {};
  const sidebarData = bbaContent?.sidebar || {};
          ...
          <div className="flex flex-col md:flex-row md:gap-6 gap-4">
            ...
            <div className="border border-[#e9e9e9] bg-white rounded-lg p-6 md:mx-0 mx-[16px]">
              <motion.div initial={false} animate={{ opacity: 1 }} transition={{ duration: 0.5 }}>
                ...
                {/* ── ELIGIBILITY (was Specializations) ───────────────────── */}
                {hasSpecializations && (
