...
export default function ComparisonMatrix({ data }) {
  ...
  const rows = [...(data.comparisonRows || [])].sort((a, b) => (a.order ?? 0) - (b.order ?? 0));
      ...
      >
        ...
        <motion.div variants={fadeInUpVariants} className="text-center mb-8 md:mb-16 relative">
          ...
          <div className="absolute lg:-top-8 -top-4 left-0 right-0 flex items-center justify-center pointer-events-none overflow-hidden">
            ...
            >
              {data.comparisonBackgroundWatermarkText}
            </h3>
          </div>

          <h2 className="relative text-3xl md:text-3xl font-bold text-gray-900 mb-4">{data.comparisonHeading}</h2>
