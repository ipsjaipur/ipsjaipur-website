...
const FALLBACK_STATS = [
  { value: '26 LPA+', label: 'Highest Package Offered' },
  { value: '5.5 LPA+', label: 'Average Package Offered' },
  { value: '100+', label: 'Top Recruiting Companies' },
];
...
export default function Placements({ data }) {
  const heading = data?.placementsHeading || 'Our Placements';
  const subText =
    data?.placementsSubText ||
    'Strong placement support with dedicated career guidance, complemented by regular workshops, seminars, and industry interaction sessions.';

  const stats =
    data?.placementsStats?.length > 0
      ? [...data.placementsStats].sort((a, b) => (a.order ?? 0) - (b.order ?? 0))
      : FALLBACK_STATS;

  const companyLogos =
    data?.companyLogos?.length > 0
      ? [...data.companyLogos].sort((a, b) => (a.order ?? 0) - (b.order ?? 0))
      : FALLBACK_LOGOS.map((l) => ({ ...l, logo: `${process.env.NEXT_PUBLIC_IMG_PATH}${l.logo}` }));
  ...
  const sliderSettings = {
    ...
    responsive: [
      ...
      { breakpoint: 420, settings: { slidesToShow: 1, slidesToScroll: 1, arrows: false, dots: true } },
      { breakpoint: 320, settings: { slidesToShow: 1, slidesToScroll: 1, arrows: false, dots: true } },
    ],
  };
      ...
      >
        ...
        <div className="flex lg:px-0 px-[16px] flex-col lg:flex-row justify-between mb-[24px] md:mb-[50px] items-start lg:items-center gap-6 lg:gap-8">
          <motion.div className="w-full lg:max-w-[500px]" variants={fadeInLeftVariants}>
            ...
            <p className="text-white text-[14px] md:text-[16px] font-medium figtree-font">{subText}</p>
          </motion.div>
          ...
          >
            {stats.map((stat, idx) => (
              <motion.div
                key={stat._id || idx}
                className="bg-white rounded-xl px-2 sm:px-6 py-4 min-w-[100px] sm:min-w-[150px] text-center shadow-lg"
              >
                <div className="text-[#FF6B00] text-[18px] sm:text-[32px] font-extrabold figtree-font leading-none">
                  {stat.value}
                </div>
                <div className="text-gray-600 text-[11px] sm:text-[14px] font-bold figtree-font mt-2">{stat.label}</div>
              </motion.div>
            ))}
          </motion.div>
        </div>
        ...
        <motion.div className="placement-slider lg:px-0 px-[8px]" variants={fadeInUpVariants}>
          <Slider {...sliderSettings}>
            {placementStudents.map((student, index) => (
              <div key={student._id || index} className="px-2 sm:px-3">
                <motion.div className="rounded-xl overflow-hidden shadow-lg transition-shadow duration-300">
                  <img src={student.image} alt={student.name} className="w-full h-auto object-cover" />
                </motion.div>
              </div>
            ))}
          </Slider>
