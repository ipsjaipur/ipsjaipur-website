...
export async function GET(request) {
  try {
    ...
    console.error('[BBA-CONTENT GET]', error);
    return NextResponse.json(
      { success: false, message: 'Failed to fetch BBA content' },
      { status: 500 }
    );
  }
}
...
export async function PUT(request) {
  try {
    ...
    const updated = await BBAPageContent.findOneAndUpdate(
      { section },
      { $set: { section, ...fields } },
    ...
    console.error('[BBA-CONTENT PUT]', error);
    return NextResponse.json(
      { success: false, message: error.message || 'Failed to update BBA content' },
      { status: 500 }
    );
  }
}
