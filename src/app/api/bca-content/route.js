...
export async function GET(request) {
  try {
    ...
    console.error('[BCA-CONTENT GET]', error);
    return NextResponse.json(
      { success: false, message: 'Failed to fetch BCA content' },
      { status: 500 }
    );
  }
}
...
export async function PUT(request) {
  try {
    ...
    const updated = await BCAPageContent.findOneAndUpdate(
      { section },
      { $set: { section, ...fields } },
    ...
    console.error('[BCA-CONTENT PUT]', error);
    return NextResponse.json(
      { success: false, message: error.message || 'Failed to update BCA content' },
      { status: 500 }
    );
  }
}
